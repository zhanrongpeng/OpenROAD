...
int lefiNoiseVictim::numNoises() const
{
  return numNoises_;
}

double lefiNoiseVictim::noise(int index) const
{
  return noises_[index];
}

double lefiNoiseVictim::length() const
{
  return length_;
}
...
void lefiNoiseResistance::Init()
{
  ...
  numVictims_ = 0;
  victimsAllocated_ = 2;
  victims_ = (lefiNoiseVictim**) lefMalloc(sizeof(lefiNoiseVictim*) * 2);
}
...
int lefiNoiseResistance::numVictims() const
{
  return numVictims_;
}

lefiNoiseVictim* lefiNoiseResistance::victim(int index) const
{
  return victims_[index];
}

int lefiNoiseResistance::numNums() const
{
  return numNums_;
}

double lefiNoiseResistance::num(int index) const
{
  return nums_[index];
}
...
void lefiNoiseEdge::Init()
{
  ...
  resistances_
      = (lefiNoiseResistance**) lefMalloc(sizeof(lefiNoiseResistance*) * 2);
}
...
void lefiCorrectionResistance::Init()
{
  ...
  victimsAllocated_ = 2;
  victims_
      = (lefiCorrectionVictim**) lefMalloc(sizeof(lefiCorrectionVictim*) * 2);
}
...
void lefiCorrectionResistance::addVictimLength(double d)
{
  ...
  if (numVictims_ == victimsAllocated_) {
    ...
    ne = (lefiCorrectionVictim**) lefMalloc(sizeof(lefiCorrectionVictim*)
                                            * max);
    max /= 2;
    for (i = 0; i < max; i++) {
      ne[i] = victims_[i];
...
void lefiCorrectionEdge::Init()
{
  ...
  numResistances_ = 0;
  resistancesAllocated_ = 2;
  resistances_ = (lefiCorrectionResistance**) lefMalloc(
      sizeof(lefiCorrectionResistance*) * 2);
}
