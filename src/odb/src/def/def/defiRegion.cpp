...
defiRegion::defiRegion(defrData* data) : defData(data)
{
  Init();
}
...
void defiRegion::addProperty(const char* name,
                             const char* value,
                             const char type)
{
  int len;
...
void defiRegion::addNumProperty(const char* name,
                                const double d,
                                const char* value,
                                const char type)
{
...
void defiRegion::print(FILE* f) const
{
  ...
  for (i = 0; i < numRectangles(); i++) {
    fprintf(f, " %d %d %d %d", xl(i), yl(i), xh(i), yh(i));
  }
  fprintf(f, "\n");
}
