...
defiGroup::defiGroup(defrData* data) : defData(data)
{
  Init();
}
...
void defiGroup::regionRects(int* size,
                            int** xl,
                            int** yl,
                            int** xh,
                            int** yh) const
...
void defiGroup::addNumProperty(const char* name,
                               const double d,
                               const char* value,
                               const char type)
{
...
const char* defiGroup::propName(int index) const
{
  ...
  if (index < 0 || index >= numProps_) {
    sprintf(msg,
            "ERROR (LEFPARS-6050): The index number %d given for the GROUP "
