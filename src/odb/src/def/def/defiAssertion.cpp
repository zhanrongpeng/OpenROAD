...
defiAssertion::defiAssertion(defrData* data) : defData(data)
{
  Init();
}
...
void defiAssertion::clear()
{
  ...
  for (i = 0; i < numItems_; i++) {
    if (itemTypes_[i] == 'p') {
      ...
      defiError(0,
                6009,
                "ERROR (DEFPARSE-6009): An invalid attribute type has "
                "encounter while cleanning the memory.",
                defData);
    }
...
void defiAssertion::setWiredlogic(const char* name, double dist)
{
  ...
  if (isDelay_) {
    defiError(
        0,
        6201,
        "ERROR (DEFPARS-6201): Unable to process the DEF file. Both WIREDLOGIC "
...
void defiAssertion::setSum()
{
  if (isDiff_) {
    defiError(
        0,
        6202,
        "ERROR (DEPPARS-6202): Unable to process the DEF file. Both SUM and "
...
void defiAssertion::path(int index,
                         char** fromInst,
                         char** fromPin,
                         char** toInst,
                         char** toPin) const
...
void defiAssertion::addPath(const char* fromInst,
                            const char* fromPin,
                            const char* toInst,
                            const char* toPin)
{
