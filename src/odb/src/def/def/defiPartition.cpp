...
defiPartition::defiPartition(defrData* data) : defData(data)
{
  Init();
}
...
void defiPartition::addTurnOff(const char* setup, const char* hold)
{
  if (*setup == ' ') {
    ...
    defiError(0,
              6100,
              "ERROR (DEFPARS-6100): The value spefified for PARTITION SETUP "
              "is invalid. The valid value for SETUP is 'R' or 'F'. Specify a "
              "valid value for SETUP and then try again.",
              defData);
  }
  ...
  if (*hold == ' ') {
    ...
    defiError(0,
              6101,
              "ERROR (DEFPARS-6101): The value spefified for PARTITION HOLD is "
              "invalid. The valid value for HOLD is 'R' or 'F'. Specify a "
              "valid value for HOLD and then try again.",
              defData);
  }
}
