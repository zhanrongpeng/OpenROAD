...
defiTimingDisable::defiTimingDisable(defrData* data) : defData(data)
{
  Init();
}
...
void defiTimingDisable::setFromTo(const char* fromInst,
                                  const char* fromPin,
                                  const char* toInst,
                                  const char* toPin)
{
...
void defiTimingDisable::print(FILE* f) const
{
  if (hasMacroFromTo()) {
    ...
    fprintf(f,
            "TimingDisable macro '%s' from '%s' to '%s'\n",
            fromInst_,
            fromPin_,
            toPin_);
    ...
    defiError(0,
              6170,
              "ERROR (DEFPARS-6170): The TimingDisable type is invalid. The "
              "valid types are FROMPIN, & THRUPIN. Specify the valid type and "
              "then try again.",
              defData);
  }
}
