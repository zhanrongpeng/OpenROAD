 ...
 * class defiComponent
 **********************************************************/
defiComponent::defiComponent(defrData* data) : defData(data)
{
  Init();
}
...
// 5.7
void defiComponent::setRouteHalo(int haloDist,
                                 const char* minLayer,
                                 const char* maxLayer)
{
  haloDist_ = haloDist;
...
{
  fprintf(fout, "Component id '%s' name '%s'", id(), name());
  if (isPlaced()) {
    fprintf(fout,
            " Placed at %d,%d orient %s",
            placementX(),
            placementY(),
            placementOrientStr());
  }
...
void defiComponent::addProperty(const char* name,
                                const char* value,
                                const char type)
{
  int len = strlen(name) + 1;
...
void defiComponent::addNumProperty(const char* name,
                                   const double d,
                                   const char* value,
                                   const char type)
{
