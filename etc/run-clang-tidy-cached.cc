...
static constexpr ConfigValues kConfig = {
    // Configuration for your project goes here. Makes it simple to switch to an
    // updated run-clang-tidy-cached.cc: only need to transfer this block.
    .start_dir = "src/",
    .file_exclude_re
...
{
  return std::any_of(options.begin(),
                     options.end(),
 ...
 private:
  ...
  fs::path AssembleProjectCacheDir(const std::string& cache_prefix) const
  {
    ...
    const std::string major_version
        = std::regex_search(
              version, version_match, std::regex{"version ([0-9]+)"})
              ? version_match[1].str()
              : "UNKNOWN";
    ...
    return cache_dir
           / fs::path(cache_prefix + "v" + major_version + "_"
...
class FileGatherer
{
 public:
                      ...
                      std::string_view symlink_summary) const
  {
    ...
    std::stable_sort(by_count.begin(),
                     by_count.end(),
